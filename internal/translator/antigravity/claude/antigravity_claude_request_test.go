...
// Retain the fork regression name while the accepted native-signature case is
// covered separately. Raw signatures from another provider must still drop.
func TestConvertClaudeRequestToAntigravity_CacheModeDropsRawSignature(t *testing.T) {
	TestConvertClaudeRequestToAntigravity_CacheModeDropsInvalidSignature(t)
}
