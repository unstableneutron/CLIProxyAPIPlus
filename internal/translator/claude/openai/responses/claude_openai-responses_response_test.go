...
func TestConvertClaudeResponseToOpenAIResponses_AggregatesTextBlocksUntilMessageStop(t *testing.T) {
	...
	var addedOutputIndices []int64
	var completed gjson.Result
	for _, output := range outputs {
		event, data := parseClaudeResponsesSSEEvent(t, output)
		counts[event]++
		if event == "response.output_item.added" {
			addedOutputIndices = append(addedOutputIndices, data.Get("output_index").Int())
		}
	...
	if len(addedOutputIndices) != 2 || addedOutputIndices[0] != 0 || addedOutputIndices[1] != 1 {
		t.Fatalf("response.output_item.added indices = %v, want contiguous [0 1]", addedOutputIndices)
	}
	...
	if got := completed.Get("response.output.#").Int(); got != 2 {
		t.Fatalf("completed output items = %d, want 2: %s", got, completed.Get("response.output").Raw)
	}
	if got := completed.Get("response.output.0.content.0.text").String(); got != "**Compare competitors**\n- Qwen 3.7 Max leads." {
		t.Fatalf("completed message text = %q, want aggregated text", got)
	}
	...
	if got := completed.Get("response.output.1.action.query").String(); got != "Qwen3" {
		t.Fatalf("completed search query = %q, want Qwen3", got)
	}
...
func TestConvertClaudeResponseToOpenAIResponses_ServerToolsSurfaceWithoutOutputIndexGaps(t *testing.T) {
	...
	if got := completed.Get("response.output.0.content.0.text").String(); got != "Searching. Found it." {
		t.Fatalf("completed message text = %q, want aggregated search-surrounding text", got)
	}
}
...
func TestClaudeResponsesAggregatedTextRetainsIncompleteStatus(t *testing.T) {
	chunks := [][]byte{
		[]byte(`data: {"type":"message_start","message":{"id":"aggregate-incomplete"}}`),
		...
		[]byte(`data: {"type":"content_block_stop","index":0}`),
		[]byte(`data: {"type":"content_block_start","index":1,"content_block":{"type":"text"}}`),
		[]byte(`data: {"type":"content_block_delta","index":1,"delta":{"type":"text_delta","text":"unfinished"}}`),
		[]byte(`data: {"type":"content_block_stop","index":1}`),
		[]byte(`data: {"type":"message_delta","delta":{"stop_reason":"max_tokens"}}`),
		[]byte(`data: {"type":"message_stop"}`),
	}
	var added, done int
	...
	for _, chunk := range translateClaudeResponsesStreamThroughRegistry(chunks) {
		...
		case "response.output_item.done":
			...
			if data.Get("item.status").String() != "incomplete" || data.Get("item.content.0.text").String() != "First unfinished" {
				t.Fatalf("unexpected aggregated item: %s", data.Raw)
			}
		case "response.completed":
			t.Fatal("max_tokens must not become response.completed")
		case "response.incomplete":
			terminal = data.Get("response")
		}
	}
	if added != 1 || done != 1 || terminal.Get("output.#").Int() != 1 {
		t.Fatalf("expected one aggregated item: added=%d done=%d terminal=%s", added, done, terminal.Raw)
	}
	if terminal.Get("incomplete_details.reason").String() != "max_output_tokens" || terminal.Get("output.0.status").String() != "incomplete" {
		t.Fatalf("incomplete details lost: %s", terminal.Raw)
	}
}
