...
func (s *Server) codexAlphaSearch(c *gin.Context) {
	...
	for {
		...
		var errSelect error
		if homeEnabled {
			selection, errSelect = s.handlers.AuthManager.SelectHomeAuthByKind(ctx, "codex", model, auth.AuthKindOAuth, selectionOpts)
			if selection != nil {
		...
		req, errRequest := s.handlers.AuthManager.NewHttpRequest(attemptCtx, selected, http.MethodPost, upstreamURL, upstreamRequestBody, headers)
		if errRequest != nil {
			lastErr = errRequest
			...
			endAttempt("request_build_failed")
			if homeEnabled {
		...
		resp, errRequest := s.handlers.AuthManager.HttpRequest(attemptCtx, selected, req)
		if errRequest != nil {
