...
func TestPrepareAntigravityGeminiReasoningReplayPayloadAppendsStaleThoughtSignatureWithoutNullParts(t *testing.T) {
	...
	item := []byte(`{"type":"thought_signature","contentIndex":8,"partIndex":3,"thoughtSignature":"sig-text-valid-len"}`)
	...
	if got := parts[1].Get("thoughtSignature").String(); got != "sig-text-valid-len" {
		t.Fatalf("thoughtSignature = %q, want sig-text-valid-len; body=%s", got, out)
	}
}
