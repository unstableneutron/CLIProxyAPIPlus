...
func (t *utlsRoundTripper) createConnection(ctx context.Context, host, addr string) (*http2.ClientConn, error) {
	...
	profile := t.profile
	if strings.TrimSpace(profile.Name) == "" && strings.TrimSpace(profile.RawClientHelloHex) == "" {
		profile = utlsProfile{Name: utlsProfileChromeAuto, HelloID: tls.HelloChrome_Auto}
	}
	tlsConn, err := newProfiledUTLSConn(conn, host, profile)
	if err != nil {
		if errClose := conn.Close(); errClose != nil {
			log.Debugf("utls: close connection after profile failure: %v", errClose)
		}
		return nil, err
	}
...
func NewUtlsHTTPClient(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth, timeout time.Duration) *http.Client {
	...
	var chromeRT http.RoundTripper = newUtlsRoundTripper(proxyURL, profile)
