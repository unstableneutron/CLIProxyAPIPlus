...
func TestCursorExecuteToolResultUsesColdContinuation(t *testing.T) {
	...
	processCount := 0
	e := newCursorExecutorHarness(func(_ context.Context, _ cursorStream, _ map[string][]byte, _ anyMCPTools, onText func(string, bool), onMcpExec func([]pendingMcpExec), toolResultCh <-chan []toolResultInfo, _ *cursorTokenUsage, _ func([]byte), _ func(), _ *cursorRawProtoLogger) error {
		if onMcpExec == nil {
...
func TestFlattenConversationIntoUserTextPreservesToolResult(t *testing.T) {
	...
	for _, want := range []string{
		...
		`Assistant called tool read (call_read) with arguments: {"path":"README.md"}`,
		"Tool result (call_read): project contents",
	} {
		if !strings.Contains(parsed.UserText, want) {
...
func TestFlattenConversationIntoUserTextPreservesToolCallOrderAndCompleteUTF8Result(t *testing.T) {
	...
	first := strings.Index(parsed.UserText, "Assistant called tool first (call_first)")
	second := strings.Index(parsed.UserText, "Assistant called tool second (call_second)")
	firstResult := strings.Index(parsed.UserText, "Tool result (call_first)")
	secondResult := strings.Index(parsed.UserText, "Tool result (call_second)")
	...
	if !strings.Contains(parsed.UserText, largeResult) || !strings.Contains(parsed.UserText, "called tool first") || !strings.Contains(parsed.UserText, `arguments: {"n":1}`) {
...
func TestCursorExecuteStreamOpenAICancellationAfterFirstChunkStopsSession(t *testing.T) {
	processorExited := make(chan struct{})
	stream := newFakeCursorStream()
	e := newCursorExecutorHarness(func(ctx context.Context, _ cursorStream, _ map[string][]byte, _ anyMCPTools, onText func(string, bool), _ func([]pendingMcpExec), _ <-chan []toolResultInfo, _ *cursorTokenUsage, _ func([]byte), _ func(), _ *cursorRawProtoLogger) error {
		onText("first", false)
		<-ctx.Done()
		close(processorExited)
		return ctx.Err()
