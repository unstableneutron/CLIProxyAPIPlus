...
func (e *CodexWebsocketsExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	for {
		...
		if wsErr, ok := parseCodexWebsocketError(payload); ok {
			wsErr = helps.MarkCodexMissingPreviousResponseRequestScoped(wsErr, req.Payload)
			if sess != nil {
		...
		if streamErr, terminalBody, ok := codexTerminalFailureErr(payload); ok && opts.ExecutionLifecycle != nil {
			if sess != nil {
				unlockSession()
	...
	go func() {
		...
		for {
			...
			if eventType == "response.completed" || eventType == "response.done" {
				payload = patchCodexCompletedOutput(payload, outputItemsByIndex, outputItemsFallback)
				completedPayload = normalizeCodexWebsocketCompletion(payload)
