...
The guarded `Recover Existing Release Tag` workflow may resume publication only
for the exact accepted upstream tag, commit, plan fingerprint, merged repair PR,
failed source run attempt, and immutable staged artifact recorded by the
candidate evidence. It revalidates all identities, resumes the staged release
without clobber, and then emits the exact `resume_release=true` v2 dispatch
needed to complete acceptance. It does not authenticate a new provenance root
by itself and it never handles hotfix suffixes. Conflicting drafts and
incomplete public releases remain terminal and require a separately reviewed
superseding policy.
