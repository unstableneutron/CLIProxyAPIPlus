...
if [ "${MODE}" = full ]; then
  ...
  if [ "${TOOLING_REQUIRED}" = true ]; then
    run_gate helper-tests \
      "${UPSTREAM_SYNC_HELPER_TEST_CMD:-UPSTREAM_SYNC_TOOLING_MODE=auto \"${SCRIPT_DIR}/test-upstream-sync.sh\" && UPSTREAM_SYNC_TOOLING_MODE=auto \"${SCRIPT_DIR}/test-verify-upstream-release.sh\" && \"${SCRIPT_DIR}/test-hotfix-release.sh\" && \"${SCRIPT_DIR}/test-verify-hotfix-chain.sh\" && \"${SCRIPT_DIR}/test-verify-hotfix-release.sh\" && \"${SCRIPT_DIR}/test-publish-staged-release.sh\" && \"${SCRIPT_DIR}/test-select-docker-digest-evidence.sh\" && \"${SCRIPT_DIR}/test-revalidate-release-target.sh\"}" \
      HELPER_TESTS_STATUS
    run_gate shellcheck \
