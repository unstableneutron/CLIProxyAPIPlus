...
func TestNormalizeResponsesWebsocketRequestCreateFastTier(t *testing.T) {
	...
	if model := gjson.GetBytes(normalized, "model").String(); model != "gpt-5.5" {
		t.Fatalf("model = %q, want gpt-5.5; payload=%s", model, normalized)
	}
	if tier := gjson.GetBytes(normalized, "service_tier").String(); tier != "priority" {
		t.Fatalf("service_tier = %q, want priority; payload=%s", tier, normalized)
	}
	if !bytes.Equal(lastRequest, normalized) {
		t.Fatalf("last request snapshot differs: normalized=%s last=%s", normalized, lastRequest)
	}
}

func TestNormalizeResponsesWebsocketRequestStripsInputItemMetadata(t *testing.T) {
	raw := []byte(`{"type":"response.create","model":"test-model","metadata":{"root":"kept"},"input":[{"type":"message","role":"user","metadata":{"turn":"drop"}}]}`)
	normalized, lastRequest, errMsg := normalizeResponsesWebsocketRequest(raw, nil, nil)
	if errMsg != nil {
		t.Fatalf("unexpected error: %+v", errMsg)
	}
	if !gjson.GetBytes(normalized, "metadata.root").Exists() {
		t.Fatalf("root metadata was removed: %s", normalized)
	}
	if gjson.GetBytes(normalized, "input.0.metadata").Exists() {
		t.Fatalf("input item metadata leaked: %s", normalized)
	}
	if !bytes.Equal(lastRequest, normalized) {
		t.Fatalf("last request snapshot differs: normalized=%s last=%s", normalized, lastRequest)
	}
...
func TestNormalizeResponsesWebsocketRequestDoesNotInjectPreviousResponseIDForCompactReplay(t *testing.T) {
	...
	lastResponseOutput := []byte(`[{"type":"message","id":"assistant-1"},{"type":"function_call","id":"fc-1","call_id":"call-1"}]`)
	...
	if got := len(gjson.GetBytes(normalized, "input").Array()); got != 6 {
		t.Fatalf("compact replay input len = %d, want 6: %s", got, normalized)
	}
...
func (e *websocketUpstreamDisconnectExecutor) ExecuteStream(context.Context, *coreauth.Auth, coreexecutor.Request, coreexecutor.Options) (*coreexecutor.StreamResult, error) {
	chunks := make(chan coreexecutor.StreamChunk, 1)
	chunks <- coreexecutor.StreamChunk{Payload: successfulResponsesWebsocketCompletion("resp-disconnect-fixture", "out-disconnect-fixture")}
	close(chunks)
	return &coreexecutor.StreamResult{Chunks: chunks}, nil
}
...
func TestResponsesWebsocketReplaysImmediatelyAfterPinnedAuthFailure(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			selector := &orderedWebsocketSelector{order: []string{"auth-a", "auth-b"}}
			executor := &websocketPinnedFailoverExecutor{provider: "xai", failStatus: tc.status}
			manager := coreauth.NewManager(nil, selector, nil)
			manager.RegisterExecutor(executor)
