...
func TestHomeRetryRoundUsesEarliestCredentialRetryAfter(t *testing.T) {
	...
	retryLimit := -1
	_, errExecute := manager.executeHomeOnce(context.Background(), []string{"home-retry-contract"}, cliproxyexecutor.Request{Model: "gpt"}, cliproxyexecutor.Options{}, false, 2, &retryLimit, nil)
...
func TestHomeSelectionFailureIsNotHiddenByEarlierUpstreamAttempt(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			retryLimit := -1
			_, errExecute := manager.executeHomeOnce(context.Background(), []string{"home-retry-contract"}, cliproxyexecutor.Request{Model: "gpt"}, cliproxyexecutor.Options{}, false, 2, &retryLimit, newRouteAttemptTracker())
			var authErr *Error
...
func TestHomeRetryRoundStartsImmediatelyWhenHomeReportsAvailableNextRound(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "nonstream",
			execute: func(manager *Manager, retryLimit *int) error {
				_, errExecute := manager.executeHomeOnce(context.Background(), []string{"home-retry-contract"}, cliproxyexecutor.Request{Model: "gpt"}, cliproxyexecutor.Options{}, false, 0, retryLimit, nil)
				return errExecute
			},
		},
		{
	...
	tests := []struct {
	...
	}{
		{
			...
			execute: func(manager *Manager, maxRetryCredentials int, retryLimit *int) error {
				_, errExecute := manager.executeHomeOnce(context.Background(), []string{"home-retry-contract"}, cliproxyexecutor.Request{Model: "gpt"}, cliproxyexecutor.Options{}, false, maxRetryCredentials, retryLimit, nil)
				return errExecute
			},
		},
		{
