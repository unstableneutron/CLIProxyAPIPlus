...
func (c *SessionCache) removeAliasGroupLocked(entry sessionEntry) {
	...
	for _, alias := range entry.aliases {
		...
		if !ok || current.authID != entry.authID || current.generation != entry.generation || !current.expiresAt.Equal(entry.expiresAt) ||
...
func sameSessionEntryGroup(left, right sessionEntry) bool {
	return left.authID == right.authID && left.generation == right.generation && left.expiresAt.Equal(right.expiresAt) &&
