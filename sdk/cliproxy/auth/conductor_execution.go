...
func (m *Manager) Execute(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	...
	if m.HomeEnabled() {
		...
		hadUpstreamAttempt := hasUpstreamExecutionAttempt(errHome)
		errHome = unwrapExecutionBoundaryError(errHome)
		if len(tracker.attempts) == 1 && tracker.attempts[0].status != "error" {
			var routed *routeExhaustionClonedError
			if errors.As(errHome, &routed) && routed != nil {
				errHome = unwrapExecutionBoundaryError(routed.cause)
			}
		}
		if hadUpstreamAttempt && len(tracker.attempts) > 1 {
			errHome = wrapRouteExhaustion(errHome, tracker)
		}
		return resp, errHome
	}
	...
	callerExcluded := cloneAuthIDSet(tried)
	for attempt := 0; ; attempt++ {
		resp, errExec := m.executeMixedOnce(ctx, normalized, req, opts, maxRetryCredentials, attempt, defaultRequestRetry, tracker, tried)
		if errExec == nil {
			return resp, nil
		}
		...
		if wait > 0 {
			...
			tried = cloneAuthIDSet(callerExcluded)
		}
	}
	if lastErr != nil {
		if ctx != nil {
...
func (m *Manager) ExecuteCount(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	...
	callerExcluded := cloneAuthIDSet(tried)
	for attempt := 0; ; attempt++ {
		resp, errExec := m.executeCountMixedOnce(ctx, normalized, req, opts, maxRetryCredentials, attempt, defaultRequestRetry, tracker, tried)
		if errExec == nil {
			return resp, nil
		}
		if isRequestTerminatedError(errExec) || isRequestStopError(errExec) {
...
func (m *Manager) ExecuteStream(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, error) {
	...
	for {
		result, errStream := m.executeStreamMixedOnce(ctx, normalized, req, opts, maxRetryCredentials, &homeRetryLimit, attempt, defaultRequestRetry, tracker, tried)
		if errStream == nil {
			return result, nil
		}
	...
	if lastErr != nil {
		...
		finalStreamError := func(err error) error {
			if m.HomeEnabled() && len(tracker.attempts) == 1 && tracker.attempts[0].status != "error" {
				var routed *routeExhaustionClonedError
				if errors.As(err, &routed) && routed != nil {
					return unwrapExecutionBoundaryError(routed.cause)
				}
				return unwrapExecutionBoundaryError(err)
			}
			return wrapRouteExhaustion(err, tracker)
		}
		var bootstrapErr *streamBootstrapError
		if errors.As(lastErr, &bootstrapErr) && bootstrapErr != nil {
			return streamErrorResult(bootstrapErr.Headers(), finalStreamError(lastErr)), nil
		}
		return nil, finalStreamError(lastErr)
	}
...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int, retryRound int, defaultRequestRetry int, tracker *routeAttemptTracker, persistentTried ...map[string]struct{}) (cliproxyexecutor.Response, error) {
	if len(providers) == 0 {
		return cliproxyexecutor.Response{}, &Error{Code: "provider_not_found", Message: "no provider supplied"}
	}
	...
	if len(persistentTried) > 0 && persistentTried[0] != nil {
		tried = persistentTried[0]
	}
...
func (m *Manager) executeCountMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int, retryRound int, defaultRequestRetry int, tracker *routeAttemptTracker, persistentTried ...map[string]struct{}) (cliproxyexecutor.Response, error) {
	if len(providers) == 0 {
		return cliproxyexecutor.Response{}, &Error{Code: "provider_not_found", Message: "no provider supplied"}
	}
	routeModel := authSelectionModelFromOptions(opts, req.Model)
...
func (m *Manager) executeStreamMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int, homeRetryLimit *int, retryRound int, defaultRequestRetry int, tracker *routeAttemptTracker, persistentTried ...map[string]struct{}) (*cliproxyexecutor.StreamResult, error) {
	...
	for {
		...
		pickExcluded := tried
		if allowSameAuthRetry {
			pickExcluded = cloneAuthIDSet(tried)
			delete(pickExcluded, lastHomeAuthID)
		}
		pickOpts = withExcludedAuthIDs(pickOpts, pickExcluded)
...
func cloneAuthIDSet(ids map[string]struct{}) map[string]struct{} {
