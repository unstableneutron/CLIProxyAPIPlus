...
func (s *SessionAffinitySelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth) (*Auth, error) {
	...
	if cachedAuthID, ok := s.cache.GetAndRefresh(cacheKey); ok {
		...
		if cachedAuthID, ok := s.cache.Get(fallbackKey); ok {
			for _, auth := range available {
				if auth.ID == cachedAuthID && (!isSubagent || s.subagentAffinity) {
					entry.Infof("session-affinity: concurrent cache hit | session=%s auth=%s provider=%s model=%s", truncateSessionID(primaryID), auth.ID, provider, model)
					bind(auth.ID)
					return auth, nil
				}
...
func (s *SessionAffinitySelector) OnResult(res Result) {
	...
	if res.Success {
		if fallbackKey != "" {
			primaryAuth, primaryBound := s.cache.Get(cacheKey)
			fallbackAuth, fallbackBound := s.cache.Get(fallbackKey)
			if (!primaryBound || primaryAuth == res.AuthID) && (!fallbackBound || fallbackAuth == res.AuthID) {
...
func (s *SessionAffinitySelector) pickLCP(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth, entry *log.Entry) (*Auth, bool, error) {
	...
	available, errAvailable := getAvailableAuthsAcrossPriorities(availabilityCandidates, provider, model, time.Now(), extractExcludedAuthIDs(opts.Metadata))
	if errAvailable != nil {
...
// These compatibility predicates describe the provider-aware policy that
// preceded the configurable unified subagent-affinity switch. Keep them for
// sync composition checks that still classify the legacy policy.
...
func allowsSubagentAuthInheritance(auth *Auth, model string) bool {
	...
	return !isNonInheritingModel(model)
}
