...
func TestSessionAffinityMixedPoolClaudeInheritsWhileAntigravityIsolates(t *testing.T) {
	...
	if !allowsSubagentAuthInheritance(&Auth{Provider: "claude"}, "claude-3-7-sonnet") {
		t.Fatal("legacy Claude subagent policy should inherit the parent credential")
	}
	if allowsSubagentAuthInheritance(&Auth{Provider: "antigravity"}, "claude-3-7-sonnet") {
		t.Fatal("legacy Antigravity subagent policy should isolate the parent credential")
	}
}
