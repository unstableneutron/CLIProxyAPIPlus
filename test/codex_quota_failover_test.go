		...
		t.Run(transport, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if transport == "sse" {
					...
					if _, errWrite := fmt.Fprintf(w, "data: %s\n\ndata: %s\n\ndata: %s\n\n", created, textDelta, terminal); errWrite != nil {
				...
				for _, event := range []string{created, textDelta, terminal} {
			...
			if !strings.Contains(string(payload), "partial answer") || quotaErr == nil {
