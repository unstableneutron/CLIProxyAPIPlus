...
Hotfix finalization publishes the deterministic final-plan/receipt Actions artifact before the immutable release receipt. A rerun may retain an earlier evidence attempt only within the same workflow run: the receipt's run/attempt chooses the exact artifact, while the canonical run must ultimately complete successfully at that attempt or a later one. Both current admission and recursive historical verification enforce this relationship and the artifact's exact run, head SHA, bytes, digest, file set, receipt, and final plan. A different run, a future attempt, replaced receipt, missing artifact, or drifted regeneration is permanently ineligible.
