...
Only `.github/workflows/upstream-sync-v2.yml` (upstream) and `.github/workflows/hotfix-release.yml` (hotfix), running from exact `main`, are accepted deployment release workflows. Other sync/tag workflows are not deployment paths. Legacy hotfix receipt schema 1 is accepted only for suffix `.1` directly above its recorded `.0` upstream root. Schema 2 hotfixes are recursively verified through every consecutive immediate parent to that same root, including receipt-asset, workflow-run, Actions-artifact, checksum, ancestry, state, and historical GHCR identities. Traversal rejects cycles and is bounded to 32 hotfix nodes; missing historical evidence is never treated as retryable publication lag.

Hotfix finalization publishes the deterministic final-plan/receipt Actions artifact before the immutable release receipt. A rerun may retain an earlier evidence attempt only within the same workflow run, and only when that exact earlier attempt ended in `failure`, `cancelled`, or `timed_out`: the receipt's run/attempt chooses the exact artifact, while the canonical run must ultimately complete successfully at that attempt or a later one. Both current admission and recursive historical verification enforce this relationship and the artifact's exact run, head SHA, bytes, digest, allowlisted and size-bounded file set, receipt, and final plan. A successful evidence attempt, different run, future attempt, replaced receipt, missing artifact, oversized archive member, or drifted regeneration is permanently ineligible.
